...
export default function App() {
  ...
  useEffect(() => {
    ...
    const rawHighlightIds = params.get('highlight') || params.get('ids') || '';
    const parsedHighlightIds = rawHighlightIds
      .split(',')
      .map((id) => id.trim())
      .filter(Boolean);

    setHighlightIds(parsedHighlightIds);
  ...
  const isHighlightPage = highlightIds.length > 0;
      ...
      <div className="max-w-6xl mx-auto px-6 pt-6 md:pt-12">
        {loading ? (
          ...
          <div className="space-y-8" id="main-content">
            <header className="flex flex-col items-center text-center space-y-3 max-w-3xl mx-auto">
              <div className="inline-flex items-center gap-1.5 px-3 py-0.5 bg-amber-400 text-slate-950 font-mono text-xs tracking-[0.25em] font-black uppercase shadow-md rotate-[-1deg]">
                Weekly Prototype Lab
              </div>

              <h1 className="text-2xl sm:text-3xl md:text-4xl font-black font-display tracking-tight text-white uppercase leading-none">
                Cardboard Digital Studio
              </h1>

              <p className="max-w-sm text-sm md:text-base text-white/80 font-light leading-relaxed">
                A focused reel of selected prototypes.
              </p>
            </header>
            ...
            <Highlight
              ...
              onBackToAllProjects={handleBackToAllProjects}
            />

            <footer className="pt-8 border-t border-dashed border-white/10 text-center space-y-4 font-mono text-[9px] tracking-wider text-white/40">
              <div className="flex justify-center">
                <SocialLinks />
              </div>
              <p>Cardboard Digital Studio © 2026. Made with high-fidelity blueprint aesthetics.</p>
            </footer>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-14 items-start">
            <div className="md:col-span-5 md:sticky md:top-8 space-y-8">
              <header className="flex flex-col items-center md:items-start text-center md:text-left space-y-5">
                ...
                <div className="pt-2 border-t border-dashed border-white/15 w-full self-center md:self-start">
                  <SocialLinks />
                </div>
              </header>
